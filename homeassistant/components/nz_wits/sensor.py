...
async def async_setup_entry(
    ...
    for schedule_type, details in SCHEDULE_TYPES.items():
        if update_options.get(
            schedule_type, False
        ):  # Default to False if somehow not in options
            entities.append(
...
class WitsPriceSensor(CoordinatorEntity[WitsDataUpdateCoordinator], SensorEntity):
    ...
    def __init__(
        ...
        config_entry_unique_id = (
            config_entry.unique_id or f"fallback_unique_id_{config_entry.entry_id}"
        )
    ...
    def native_value(self) -> float | None:
        ...
        if (
            ...
            or self._schedule_type not in self.coordinator.data
        ):
            return None
        ...
        if (
            not schedule_data
            or not isinstance(schedule_data, list)
            or len(schedule_data) == 0
        ):
            return None
    ...
    def extra_state_attributes(self) -> dict[str, any] | None:
        ...
        attributes = {
            ...
            "schedule_name": SCHEDULE_TYPES[self._schedule_type][
                "name"
